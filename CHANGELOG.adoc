...
* Rename `add.sh` to `add-via-payara.sh` and `delete.sh` to `delete-via-payara.sh` in the Jakarta Data example demo.
* Rename `generate-token.sh` to `generate-token-via-payara.sh` in the MicroProfile JWT Bridge example demo.
* Add `add-via-glassfish.sh` and `delete-via-glassfish.sh` to the Jakarta Data demo.
* Add `generate-token-via-glassfish.sh` to the MicroProfile JTW Bridge example demo.
* Update the `beans.xml` file in all example demos
- Set the `bean-discovery-mode` attribute to `annotated` instead of `all`.
* Add the appropriate scope, i.e., `@ApplicationScoped`, to all beans where necessary.

=== January 14, 2026

* Rename `build-and-exec.sh` to `payara-build-and-exec.sh` in all example demos.
* Add `glassfish-build-and-exec.sh` to all example demos.
* Upgrade `io.smallrye.config:smallrye-config-core` from 3.8.3 to 3.15.0.
* Test Jakarta EE 11 demo applications with a special build of GlassFish 8.0.0-M15.

=== December 31, 2025

* Test Jakarta EE 11 demo applications with Payara Micro 7.2025.2.
* Upgrade `org.eclipse.jnosql.databases:jnosql-mongodb` from 1.1.0 to 1.1.1.
* Upgrade `io.smallrye.config:smallrye-config-core` from 3.8.2 to 3.8.3.
* Upgrade `org.glassfish:mojarra-parent` from 4.1.0 to 4.1.1.
* Upgrade `org.jboss.weld.se:weld-se-shaded` from 5.1.2.Final to 5.1.3.Final.
* Upgrade `de.flapdoodle.embed:de.flapdoodle.embed.mongo` from 4.13.1 to 4.22.0.
* Security upgrade `org.glassfish.soteria:jakarta.security.enterprise` from 3.0.3 to 3.0.4.
* Security upgrade `org.glassfish.jersey.core:jersey-client` from 4.0.0-M1 to 4.0.0.

=== December 5, 2025

* Add Payara Micro and Payara Qube plugins in parent `pom.xml` file.

=== December 2, 2025

* Test Jakarta EE 11 demo applications with Payara Micro 7.2025.1.
* Create `build-and-exec.sh` scripts for each demo application.
* Bump `org.glassfish.jersey.core:jersey-common` and `org.glassfish.jersey.core:jersey-client` from 4.0.0-M1 to 4.0.0.
* Bump `org.jboss.resteasy:resteasy-client` from 7.0.0.Alpha1 to 7.0.0.Final.

=== September 26, 2025

* Update README.adoc to add presentation at https://www.dubjug.org/[Dublin Java Users Group]
