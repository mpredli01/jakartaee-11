...
This project is a demonstration on how to use the https://jakarta.ee/specifications/concurrency/[Jakarta Concurrency] specification. The application is a task creator that allows you to add tasks that are handled immediately or with a delay of three or eight seconds.
...
`$ java -jar <path>/payara-micro-<version>.jar target/concurrency-1.0.0.war`
...
http://localhost:8080/concurrency-1.0.0[Main Application]
